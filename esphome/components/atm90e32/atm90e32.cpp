  ...
  if (this->restored_offset_calibration_ || this->restored_power_offset_calibration_ ||
    ...
    this->set_interval("atm90e32_calibration_log", 1000, [this]() {
      ...
      if (api::global_api_server != nullptr && api::global_api_server->is_connected()) {
        ...
        if (offset_mismatch) {
          ESP_LOGW(TAG, "[CALIBRATION][%s] Offset mismatch: using flash values.", cs);
        ...
        if (power_mismatch) {
          ESP_LOGW(TAG, "[CALIBRATION][%s] Power offset mismatch: using flash values.", cs);
          ESP_LOGW(TAG, "[CALIBRATION][%s]                 Phase A    Phase B    Phase C", cs);
        ...
        if (gain_mismatch) {
          ESP_LOGW(TAG, "[CALIBRATION][%s] Gain mismatch: using flash values.", cs);
          ESP_LOGW(TAG, "[CALIBRATION][%s]                 Phase A    Phase B    Phase C", cs);
        ...
        if (!this->enable_gain_calibration_) {
          ...
          ESP_LOGI(TAG, "[CALIBRATION][%s] Gain calibration loaded and verified successfully.", cs);
        }
...
void ATM90E32Component::clear_gain_calibrations() {
  GainCalibration tmp[3];
  if (!this->gain_calibration_pref_.load(&tmp)) {
  ...
  for (int phase = 0; phase < 3; phase++) {
    gain_phase_[phase].voltage_gain = this->config_gain_phase_[phase].voltage_gain;
    gain_phase_[phase].current_gain = this->config_gain_phase_[phase].current_gain;
  }
...
void ATM90E32Component::clear_offset_calibrations() {
  OffsetCalibration tmp[3];
  if (!this->offset_pref_.load(&tmp)) {
...
void ATM90E32Component::clear_power_offset_calibrations() {
  PowerOffsetCalibration tmp[3];
  if (!this->power_offset_pref_.load(&tmp)) {
    ESP_LOGI(TAG,
